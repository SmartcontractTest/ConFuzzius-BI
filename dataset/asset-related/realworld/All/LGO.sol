        ...
        if (now < BONUS_DATES[3] &&
            ...
            for (uint8 i = 0; i < 4; i++) {
                if (now < BONUS_DATES[i] && eligibleForBonus[BONUS_DATES[i]][_from]) {
                    unspentAmounts[BONUS_DATES[i]] -= initialAllocations[_from];  //<underflow>
...
contract Legolas is EIP20 {
    ...
    function allocate(address _address, uint256 _amount, uint8 _type) public onlyOwner returns (bool success) {
        ...
        if (_type == 0) { // advisor
            ...
            require(advisorsAllocatedAmount + _amount <= ADVISORS_AMOUNT);  // <overflow> / <unchecked by osiris>
            // increase allocated amount
            advisorsAllocatedAmount += _amount;  //<overflow>
            // mark address as advisor
            advisors[_address] = true;
        } else if (_type == 1) { // founder
            // check allocated amount
            require(foundersAllocatedAmount + _amount <= FOUNDERS_AMOUNT);  // <overflow> / <unchecked by osiris>
            // increase allocated amount
            foundersAllocatedAmount += _amount;  //<overflow>
            // mark address as founder
            founders[_address] = true;
        } else {
            // check allocated amount
            require(holdersAllocatedAmount + _amount <= HOLDERS_AMOUNT + RESERVE_AMOUNT);  // <overflow> / <unchecked by osiris>
            // increase allocated amount
            holdersAllocatedAmount += _amount;  //<overflow>
        }
        // set allocation
        ...
        // increase balance
        balances[_address] += _amount;  //<overflow>
    ...
    function claimBonus(address _address, uint256 _bonusDate) public returns (bool success) {
        ...
        uint256 holderBonus = initialAllocations[_address] * bonusByLgo;  //<overflow>
        balances[_address] += holderBonus;  //<overflow>
        allocations[_address] += holderBonus;  //<overflow>
