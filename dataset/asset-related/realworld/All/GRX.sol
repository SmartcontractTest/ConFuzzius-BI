...
contract GRX is owned {
    ...
    function increaseTotalSupply (address target,  uint256 increaseBy )  onlyOwner {
        balanceOf[target] += increaseBy;  // <overflow>
        totalSupply += increaseBy;  // <overflow>
    ...
    function reward(address _to, uint256 _value, bool locked, string data) {
        ...
        if(!locked) {
            ...
            lockedRewardsOf[_to][msg.sender] += _value;  // <overflow>
            ...
            totalRewardIssuedOut[msg.sender]+= _value;  // <overflow>
    ...
    function unlockReward(address addr, uint256 _value) {
        ...
        totalLockedRewardsOf[addr] -= _value;  //<underflow>                         // Add the same to the recipient
        lockedRewardsOf[addr][msg.sender] -= _value;
        balanceOf[addr] += _value;  //<overflow>
    ...
    /*
     ...
     balanceOf[_from] += _value;        // <overflow>                   // Subtract from the targeted balance
     totalSupply += _value;       // <overflow>                         // Update totalSupply
     // Burn(_from, _value);
     return true;
     }
     */
    ...
    function buy() payable returns (uint256 amount){
        ...
        amount = msg.value * buyPrice;        // <overflow>              // calculates the amount

        require(balanceOf[this] >= amount);               // checks if it has enough to sell
        balanceOf[msg.sender] += amount;      // <overflow>              // adds the amount to buyer's balance
    ...
    function sell(uint256 amount) returns (uint revenue){
        ...
        balanceOf[this] += amount;       // <overflow>                   // adds the amount to owner's balance
