...
contract CCTHCoin is SafeMath{
    ...
    function updateSupply() internal returns (uint256) {
      ...
      if (now - timeOfLastHalving >= 2100000 minutes) {  //<underflow>
        reward /= 2;
        timeOfLastHalving = now;
      }

      if (now - timeOfLastIncrease >= 150 seconds) {  //<underflow>
      ...
      circulating_supply = pre_mined_supply - mined_coin_supply;  //<underflow>
    ...
    function mint(uint256 _value) {
        if(msg.sender != owner)throw;
            ...
            mined_coin_supply -= _value;  //<underflow>   // Remove from unspent supply   
