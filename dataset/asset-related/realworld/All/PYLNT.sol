...
contract PylonToken is owned {
    ...
    function burn(uint256 _value) onlyOwner returns (bool success) {
        ...
        totalSupply -= _value;  //<underflow>                    // Updates totalSupply
    ...
    function burnFrom(address _from, uint256 _value) onlyOwner returns (bool success) {
        ...
        totalSupply -= _value;  //<underflow>                              // Update totalSupply
        Burn(_from, _value);
        return true;
    }
    ...
    function buyIco(address investor) internal{
      ...
      require((balanceOf[investor] + tokens) <= maxTokens);    //<overflow>      // Check if the investor has more tokens than 5% of total supply
      require(balanceOf[this] >= tokens);             // checks if it has enough to sell
      require(pylonSelled + tokens <= totalTokensToSend);  //<overflow>  //Overflow - Check if pylons we have sell is more or equal than total tokens ew have

      balanceOf[this] -= tokens;
      balanceOf[investor] += tokens;  //<overflow> 
      amountRaised += weiAmount;   //<overflow> // update state amount raised
      pylonSelled += tokens;     //<overflow>  // Total tokens selled
    ...
    function buyPreIco(address investor) internal{
      ...
      require(pylonSelled + tokens <= bonusCap);   //<overflow> // Check if want to sell more than total tokens for pre-ico
      require(balanceOf[investor] + tokens <= maxTokens);   //<overflow> // Check if the investor has more tokens than 5% of total supply
      require(balanceOf[this] >= tokens);             // checks if it has enough to sell
