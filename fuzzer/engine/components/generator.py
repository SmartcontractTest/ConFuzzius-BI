...
class Generator:
    def __init__(self, interface, booking, bytecode, accounts, contract):
        ...
        self.booking = booking    # {'0x27e235e3': ['address'], '0x70a08231': ['address'], 'fallback': []}  = get_booking_function_from_abi(abi)
    ...
    def generate_random_individual(self):
        individual = []
        # print("@@@@@@@@ self.accounts_pool self.accounts_pool self.accounts_pool  s% !!!", self.accounts_pool)  # @@@ added by sjl at 20220905
        # print("@@@@@@@@ accounts accounts accounts  s% !!!", self.accounts)  # @@@ added by sjl at 20220905
        if "constructor" in self.interface and self.bytecode:
            ...
            for index in range(len(self.interface["constructor"])):
                ...
                # print("@@@@@@@@ constructor: arguments.append  s% !!!", arguments)  # @@@ added by sjl at 20220905
                # print("@@@@@@@@ constructor: self.accounts self.accounts  s% !!!", self.accounts)  # @@@ added by sjl at 20220905
            individual.append({
        ...
        function, argument_types = self.get_random_function_with_argument_types() #  get fun from random set(interface), interface:{"function_sinature":[para_type]}
        arguments = [function]
        # print("@@@@@@@@ bookingbookingbooking  s% !!!",  self.booking)   #    @@@ added by sjl at 20220905
        # booking_circular_buffer = CircularSet(set_size=len(self.booking), initial_set=set(self.booking))
        booking_fun_list = list(self.booking.items())   #    @@@ added by sjl at 20220905
        # print("@@@@@@@@ booking_fun_list  s% !!!", booking_fun_list)
        booking_fun = booking_fun_list[0][0]      #    @@@ added by sjl at 20220905
        # print("@@@@@@@@ booking_fun booking_fun  s% !!!", booking_fun)
        # booking_arg_types = self.booking[0][1]      #    @@@ added by sjl at 20220905
        booking_argument = [booking_fun]      #    @@@ added by sjl at 20220905
        ...
        for index in range(len(argument_types)):
            ...
            # print("@@@@@@@@ arguments.append  s% !!!", arguments)  #    @@@ added by sjl at 20220905
            # print("@@@@@@@@ self.accounts self.accounts  s% !!!", self.accounts)  #    @@@ added by sjl at 20220905
        account = self.get_random_account(function)      #    @@@ added by sjl at 20220905
        # print("@@@@@@@@ get_random_account  get_random_account s% !!!", account)  # @@@ added by sjl at 20220905
        # print("@@@@@@@@ accounts after get_random_account s% !!!", self.accounts)  # @@@ added by sjl at 20220905
        ...
        # print("@@@@@@@@ generator: booking_arguments.append to booking_arguments_list, booking_arguments_list is  s% !!!", booking_arguments_list)  # @@@ added by sjl at 20220905

        individual.append({
            "account": account,  #    @@@ added by sjl at 20220905
            "contract": self.contract,
            "amount": self.get_random_amount(function),
            "arguments": arguments,
            "booking_arguments_list" : booking_arguments_list,  #  contain funtion balanceOf and all account in self.accounts @@@ added by sjl at 20220905
        ...
        # if self.accounts_pool == {}:  # @@@ added by sjl at 20220906
        #     print("@@@@@@@@ self.accounts_pool  is empty !!!")  # @@@ added by sjl at 20220906
        # else:
        #     print("@@@@@@@@ self.accounts_pool  s% !!!", self.accounts_pool)  # @@@ added by sjl at 20220906

        # print("@@@@@@@@ self.accounts_pool self.accounts_pool self.accounts_pool  s% !!!", self.accounts_pool)  # @@@ added by sjl at 20220905
        # print("@@@@@@@@ accouts after generate_random_individual :s% !!!", self.accounts)  # @@@ added by sjl at 20220905
    ...
    def get_random_function_with_argument_types(self): # return the last one in function_circular_buffer and head it
    ...
    def get_random_amount0(self, function):
        if function in self.amounts_pool:
        ...
        return amount
    def get_random_amount(self, function): #rewrite
        if function in self.amounts_pool:
            amount = random.randint(0, 1)
    ...
    def get_random_argument(self, type, function, argument_index):
        ...
        # Address
            ...
            # Array  batchTransfer(address[] _receivers, uint256 _value)
            if "[" in type and "]" in type:
                sizes = self._get_array_sizes(argument_index, function, type)
                array = []
                for _ in range(sizes[0]):
                ...
                if len(sizes) > 1:
                    ...
                    for _ in range(sizes[1]):
                        new_array.append(array)
                    array = new_array
                # print("@@@@@generator  get_random_address: array s%",  array)
                # print("@@@@@generator  self.accounts: s%",  self.accounts)
                self.accounts = self.accounts + array
                # print("@@@@@generator  self.accounts with array: s%", self.accounts)
                self.accounts = list(set(self.accounts))
                # print("@@@@@generator  self.accounts with array and dedup: s%", self.accounts)
                # self.accounts = self.accounts.append(array)
                #
                return array
            # Single value  transfer(address _to, uint256 _value)
            else:
                if function in self.arguments_pool and argument_index in self.arguments_pool[function]:
                    # print("@@@@@  get_random_address: Single from pool s%",
                    #       self._get_random_argument_from_pool(function, argument_index))
                    # print("@@@@@  self.accounts: s%", self.accounts)
                    self.accounts.append(self._get_random_argument_from_pool(function, argument_index))
                    # print("@@@@@  self.accounts with  Single from pool: s%", self.accounts)
                    self.accounts = list(set(self.accounts))
                    # print("@@@@@  self.accounts with Single from pool and dedup: s%", self.accounts)

                    return self._get_random_argument_from_pool(function, argument_index)
                # print("@@@@@  get_random_address: Single from choice s%", random.choice(self.accounts))
                # print("@@@@@  self.accounts: s%", self.accounts)
                (self.accounts).append(random.choice(self.accounts))
                # print("@@@@@  self.accounts with Single: s%", self.accounts)
                self.accounts = list(set(self.accounts))
                # print("@@@@@  self.accounts with Single and dedup: s%", self.accounts)
                #
                return random.choice(self.accounts)
