...
from utils.utils import initialize_logger, compile, get_interface_from_abi, get_pcs_and_jumpis, get_function_signature_mapping, get_booking_function_from_abi
...
class Fuzzer:
    def __init__(self, contract_name, abi, deployment_bytecode, runtime_bytecode, test_instrumented_evm, blockchain_state, solver, args, seed, source_map=None):
        ...
        # hex_of_abi = encoding.hex_encode_abi_type(abi)
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!@@@@@@@@hex_of_abi %s", hex_of_abi)
        cfg = ControlFlowGraph()
        cfg.build(runtime_bytecode, settings.EVM_VERSION)
       # print("@@@@@@@@CFG %s", cfg.edges)


        self.contract_name = contract_name
        # print("@@@@@@@@abi %s", abi)
        self.interface = get_interface_from_abi(abi)
        self.booking = get_booking_function_from_abi(abi)  #@@@added by sjl 0905
        ...
        self.env = FuzzingEnvironment(instrumented_evm=self.instrumented_evm,
                                      ...
                                      booking = self.booking,         #    @@@ added by sjl at 20220905
    ...
    def run(self):
        # print("@@@@@@@@argsargsargsargsargs %s", self.args)
        contract_address = None
        self.instrumented_evm.create_fake_accounts()
        account_list = self.instrumented_evm.accounts
        print("@@@@@@@@create_fake_accounts create_fake_accounts create_fake_accounts %s", account_list)

        # print("@@@@@@@@argsargsargsargsargs %s", self.args)
        if self.args.source:
            # print("@@@@@@@@transaction-args.source %s", 1111111)

            for transaction in self.blockchain_state:
                # print("@@@@@@@@transactiontransactiontransaction %s", transaction)
            ...
            if "constructor" in self.interface:
                # print("@@@@@@@@ del constructor", 1111111)
                del self.interface["constructor"]

            if not contract_address:
                # print("@@@@@@@@ not contract_address", 1111111)
                if "constructor" not in self.interface:
                    print("@@@@@@@@ constructor not in !!!" , 1111111)
        ...
        generator = Generator(interface=self.interface,
                              ...
                              accounts=self.instrumented_evm.get_accounts(),
                              contract=contract_address)
        # print("@@@@@@@@ Generator  s% !!!", generator.interface)

        # Create initial population
        size = 2 * len(self.interface)
        population = Population(indv_template=Individual(generator=generator),
                                indv_generator=generator,
                                size=settings.POPULATION_SIZE if settings.POPULATION_SIZE else size).init()  #@@@@@sjl: init here!
        # print("@@@@@@@@ settings.POPULATION_SIZE  s% !!!", settings.POPULATION_SIZE)
        ...
        # print("@@@@@@@@ engine.mapping engine.mapping engine.mapping   s% !!!", engine.mapping )
        ...
        engine.run(ng=settings.GENERATIONS)   #@@@@@sjl: run here

        if self.env.args.cfg:
            # print("cfgcfgcfgcfgcfgcfgcfgcfgcfgcfgcfgcfgcfgcfgcfgcfgcfgcfg")
            if self.env.args.source:
                # print('@@@@@@@@@@@@@@@@@@@@@cfg path : s%',os.path.splitext(self.env.args.source)[0]+'-'+self.contract_name)
...
def main():
    ...
    # print("@@@@@@@@argsargsargsargsargs %s", args)
    # print("@@@@@@@@sourcesourcesourcesource %s", args.source)
        ...
        if args.source.endswith(".sol"):
            ...
            for contract_name, contract in compiler_output['contracts'][args.source].items():
                ...
                if contract['abi'] and contract['evm']['bytecode']['object'] and contract['evm']['deployedBytecode']['object']:
                    ...
                    # print("@@@@@@@@contract_name %s" ,contract_name)
                    #print("@@@@@@@@abi %s" ,contract["abi"])
                    # print("@@@@@@@@bytecode %s" , contract['evm']['bytecode']['object'])
                    # print("@@@@@@@@deployedBytecode %s" , contract['evm']['deployedBytecode']['object'])
