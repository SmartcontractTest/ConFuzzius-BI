...
class InstrumentedEVM:
    ...
    def deploy_transaction(self, input, gas_price=settings.GAS_PRICE, debug=False):
        ...
        # print('@@@@@@@ deploy_transaction s%',transaction )
        ...
        if "call_return" in global_state and global_state["call_return"] is not None \
            ...
            # print('call_returncall_returncall_returncall_return!!!!!')
    ...
    def deploy_checker(self, input, gas_price=settings.GAS_PRICE, debug=False):  #    @@@ added by sjl at 20220905
        sum_output = 0
        output_map = {}
        checker = input["checker"]
        # print('@@@@@@@ deploy_checker s%',checker)
        from_account = decode_hex(checker["from"])
        # accounts = accounts_in_env

        for data in checker["data_list"]:

            addr = '0x'+data[34:]
            # print("@@@@@@@@ output_of output_of : s% !!!", addr)


            nonce = self.vm.state.get_nonce(from_account)
            try:
                to = decode_hex(checker["to"])
            except:
                to = checker["to"]
            ...
            tx = self.vm.create_unsigned_transaction(
                    nonce=nonce,
                    gas_price=gas_price,
                    gas=checker["gaslimit"],
                    to=to,
                    value=checker["value"],
                    data=decode_hex(data),
            )
            tx = SpoofTransaction(tx, from_=from_account)
            res = self.execute(tx, debug=debug)
            if not res.is_error :
                    data_bytes = res.output
                    # print("@@@@@@@@ self.execute(tx, debug=debug) success, data_bytes is s% !!!", data_bytes)
                    output = int.from_bytes(bytes=data_bytes, byteorder="big", signed=True)
                    # print("@@@@@@@@ self.execute(tx, debug=debug) success, output is s% !!!", output)
                    output_map.update({addr:output})
                    sum_output = sum_output + output

            else:
                continue
                    # print("@@@@@@@@ self.execute(tx, debug=debug) failed  !!!")

            # print("@@@@@@@@ sum_output  sum_output sum_output s% !!!", sum_output)
            # print("@@@@@@@@ output_map  output_map output_map s% !!!", output_map)
        return sum_output, output_map
