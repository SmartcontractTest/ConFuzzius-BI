...
def get_interface_from_abi(abi):
    # print("@@@@@@@@get_interface_from_abi@@@@@@@@g@@@@@@@@g@@@@@@@@g@@@@@@@@g@@@@@@@@g")
    interface = {}
    for field in abi:
        # print("@@@@@@@@ABIABI@@@@@@@@g@@@@@@@@g@@@@@@@@g@@@@@@@@g@@@@@@@@g s%",field)
        # is_booking(field)
...
def is_booking(field):
    output_type = ['uint','uint8','uint32','uint64','uint128','uint256']
    if field['type'] == 'function' and len(field['inputs']) == 1 and field['inputs'][0]['type'] == 'address' \
            and len(field['outputs']) == 1 and field['outputs'][0]['type'] in output_type :
        # print("@@@@@@@@is_bookingis_booking@@@@@@@@g@@@@@@@@g@@@@@@@@g@@@@@@@@g@@@@@@@@g s%", field)
        return True
    else:
        return False

def get_fun_name_by_data(data,fun_map):
        _function_hash = data[:10] if data.startswith("0x") else data[:8]
        _function_hash = "fallback" if _function_hash == '' or _function_hash == '00000000' else _function_hash
        fun_name = fun_map[_function_hash]
        return  fun_name


def get_booking_function_from_abi(abi):
    # print("@@@@@@@@utilspy: fun get_booking_function@@@@@@@@g@@@@@@@@g@@@@@@@@g@@@@@@@@g@@@@@@@@g")

    booking = {}
    for field in abi:
        if  is_booking(field):
            # print("@@@@@@@@  got booking function @@@@@@@@ s%",field)
            ...
            hash = Web3.sha3(text=signature)[0:4].hex()
            # print("@@@@@@@@hash %s", hash)
            booking[hash] = function_inputs
            # print("@@@@@@@@function_inputs %s", booking[hash])


    # print("@@@@@@@@bookingbookingbookingbooking %s", booking)
    #@@@@@@@@bookingbookingbookingbooking %s {'0x70a08231': ['address']}
    return booking
